...
const KubernetesDocs = React.lazy(() => import('../components/kubernetes/KubernetesDocs'));
const AirflowDocs = React.lazy(() => import('../components/airflow/AirflowDocs'));
...
const TechnologyPage = () => {
  ...
  const getDocComponent = () => {
    switch (tech.docComponent) {
      case 'KafkaDocs': return KafkaDocs;
      case 'SparkDocs': return SparkDocs;
      case 'SQLDocs': return SQLDocs;
      case 'RedisDocs': return RedisDocs;
      case 'KubernetesDocs': return KubernetesDocs;
  ...
  const getVisualizerComponent = () => {
    switch (tech.id) {
      case 'kafka': return KafkaArchitecture;
      default: return null;
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              {tech.name}
              <span className={`text-xs px-2 py-0.5 rounded border bg-${tech.category.color}-900 text-${tech.category.color}-300 border-${tech.category.color}-700`}>
                {tech.category.name}
              </span>
            </h1>
          </div>
      ...
      <div className="flex-1 overflow-hidden bg-gray-900 relative">
        <Suspense fallback={<div className="p-8 text-gray-400">Loading content...</div>}>
          {activeTab === 'docs' ? (
            DocComponent ? (
            ...
            VisualizerComponent ? (
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-bold text-white mb-2">Visualizer</h2>

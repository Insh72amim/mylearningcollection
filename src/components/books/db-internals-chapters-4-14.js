// Database Internals Enhanced Chapters 4-14
export const dbInternalsChapters4to14 = [
  {
    id: 4,
    part: 'I',
    title: 'Implementing B-Trees',
    summary: 'Practical B-Tree implementation: page headers, binary search within nodes, propagating splits upward, and merging underflowing nodes.',
    detailedContent: 'Real databases don\'t implement textbook B-Trees. They add optimizations like prefix compression, bulk loading, and lazy deletion.',
    keyPoints: [
...
int find_key_position(BTreeNode *node, KeyType key) {
    ...
    diagram: {
      ...
      nodes: [
        { id: 'before', type: 'default', position: { x: 150, y: 100 }, data: { label: 'Full Node\n[5,10,15,20]' }, style: { background: '#dc2626', color: 'white', padding: 15, fontSize: 12 } },
        { id: 'arrow', type: 'default', position: { x: 350, y: 100 }, data: { label: 'Split at 15 →' }, style: { background: '#374151', color: 'white', padding: 12, fontSize: 11 } },
        { id: 'left', type: 'default', position: { x: 530, y: 50 }, data: { label: '[5, 10]' }, style: { background: '#10b981', color: 'white', padding: 12, fontSize: 12 } },
        { id: 'right', type: 'default', position: { x: 530, y: 150 }, data: { label: '[20]' }, style: { background: '#10b981', color: 'white', padding: 12, fontSize: 12 } },
        { id: 'promoted', type: 'default', position: { x: 700, y: 100 }, data: { label: '15 promoted\nto parent' }, style: { background: '#3b82f6', color: 'white', padding: 12, fontSize: 11 } },
      ],
      edges: []
    }
  },
  {
    ...
    summary: 'ACID transactions, write-ahead logging (WAL), checkpointing, and ARIES crash recovery algorithm.',
    detailedContent: 'Durability requires writing changes to disk before acknowledging. WAL ensures atomicity and durability without writing data pages immediately.',
    keyPoints: [
...
wal.commit_transaction(tx_id)  # Fsync happens here`
    ...
    diagram: {
      ...
      nodes: [
        { id: 'tx', type: 'default', position: { x: 50, y: 150 }, data: { label: 'Transaction' }, style: { background: '#6366f1', color: 'white', padding: 15 } },
        { id: 'wal', type: 'default', position: { x: 230, y: 150 }, data: { label: 'WAL Buffer' }, style: { background: '#f59e0b', color: 'white', padding: 15 } },
        { id: 'disk-log', type: 'default', position: { x: 410, y: 80 }, data: { label: 'WAL Disk\n(Sequential)' }, style: { background: '#dc2626', color: 'white', padding: 15, fontSize: 12 } },
        { id: 'disk-data', type: 'default', position: { x: 410, y: 220 }, data: { label: 'Data Pages\n(Random)' }, style: { background: '#10b981', color: 'white', padding: 15, fontSize: 12 } },
      ],
      edges: [
        { id: 'e1', source: 'tx', target: 'wal', label: 'Write', animated: true },
        { id: 'e2', source: 'wal', target: 'disk-log', label: 'Flush (fsync)', animated: true },
        { id:'e3', source: 'disk-log', target: 'disk-data', label: 'Lazy write', animated: false, style: { strokeDasharray: '5,5' } },
      ]
    }
  },
  {
    ...
    summary: 'Copy-on-Write B-Trees (immutable), lazy B-Trees (buffered updates), and lock-free Bw-Trees using delta records.',
    detailedContent: 'Different B-Tree variants optimize for different workloads: COW for snapshots, lazy for write batching, lock-free for concurrency.',
    keyPoints: [
  ...
  },
  {
    ...
    summary: 'LSM-Trees with memtable, SSTables, compaction strategies (size-tiered vs leveled), and Bloom filters for fast negative lookups.',
    detailedContent: 'LSM-Trees trade read performance for write throughput. Used in Cassandra, RocksDB, HBase, and LevelDB.',
    keyPoints: [
...
def read_from_sstable(key):
    ...
    diagram: {
      ...
      nodes: [
        { id: 'memtable', type: 'default', position: { x: 100, y: 50 }, data: { label: 'MemTable\n(RAM)' }, style: { background: '#dc2626', color: 'white', padding: 20, border: '2px solid #ef4444' } },
        { id: 'l0', type: 'default', position: { x: 300, y: 50 }, data: { label: 'L0\n4 SSTables\n(Overlapping)' }, style: { background: '#f59e0b', color: 'white', padding: 15, fontSize: 11 } },
        { id: 'l1', type: 'default', position: { x: 500, y: 50 }, data: { label: 'L1\n10 SSTables\n(Non-overlapping)' }, style: { background: '#3b82f6', color: 'white', padding: 15, fontSize: 11 } },
        { id: 'l2', type: 'default', position: { x: 700, y: 50 }, data: { label: 'L2\n100 SSTables' }, style: { background: '#8b5cf6', color: 'white', padding: 15, fontSize: 11 } },
      ],
      edges: [
        { id: 'e1', source: 'memtable', target: 'l0', label: 'Flush', animated: true },
        { id: 'e2', source: 'l0', target: 'l1', label: 'Compact', animated: false },
        { id: 'e3', source: 'l1', target: 'l2', label: 'Compact', animated: false },
      ]
    }
  },
  {
    ...
    summary: 'Distributed systems challenges: consistency models, failure detection, time synchronization, and distributed transactions.',
    detailedContent: 'Distributed systems introduce partial failures, network delays, and clock skew. These make strong guarantees expensive.',
    keyPoints: [
  ...
  },
  {
    ...
    summary: 'Detecting node failures using heartbeats, adaptive timeouts, and gossip protocols for group membership.',
    detailedContent: 'In distributed systems, distinguishing between slow nodes and crashed nodes is hard. False positives are inevitable.',
    keyPoints: [
  ...
  },
  {
    ...
    summary: 'Bully algorithm, ring-based election, and ZooKeeper-based stable leader election with fencing tokens.',
    detailedContent: 'Leader election ensures exactly one coordinator. Prevents split-brain and duplicate work.', 
    keyPoints: [
    ...
    sections: [
      {
        ...
        content: 'Leader gets monotonically increasing token from coordination service. Include token with requests. Server rejects requests with old tokens.',
        example: {
          language: 'python',
...
# → Storage rejects! Prevents data corruption`
  ...
  },
  {
    ...
    summary: 'Primary-backup, chain replication, and quorum-based replication. Trading consistency for availability.',
    detailedContent: 'Replication provides fault tolerance and scalability. But coordinating replicas introduces complexity.',
    keyPoints: [
  ...
  },
  {
    ...
    summary: 'Read repair, Merkle trees for efficient synchronization, and gossip protocols for state propagation.',
    detailedContent: 'Even with quorums, replicas can diverge. Anti-entropy mechanisms ensure eventual convergence.',
    keyPoints: [
    ...
    diagram: {
      ...
      nodes: [
        { id: 'root', type: 'default', position: { x: 300, y: 50 }, data: { label: 'Root\nHash(A|B)' }, style: { background: '#3b82f6', color: 'white', padding: 15, fontSize: 11 } },
        { id: 'a', type: 'default', position: { x: 150, y: 150 }, data: { label: 'A\nHash(A1|A2)' }, style: { background: '#10b981', color: 'white', padding: 12, fontSize: 11 } },
        { id: 'b', type: 'default', position: { x: 450, y: 150 }, data: { label: 'B\nHash(B1|B2)' }, style: { background: '#f59e0b', color: 'white', padding: 12, fontSize: 11 } },
        { id: 'a1', type: 'default', position: { x: 80, y: 250 }, data: { label: 'A1\nKeys 0-25' }, style: { background: '#6b7280', color: 'white', padding: 10, fontSize: 10 } },
        { id: 'a2', type: 'default', position: { x: 220, y: 250 }, data: { label: 'A2\nKeys 26-50' }, style: { background: '#6b7280', color: 'white', padding: 10, fontSize: 10 } },
      ],
      edges: [
  ...
  },
  {
    ...
    summary: 'Two-Phase Commit (2PC) protocol, coordinator recovery, and the blocking problem when coordinator fails.',
    detailedContent: '2PC achieves atomicity across nodes but blocks on coordinator failure. Three-Phase Commit tries to fix this but rarely used.',
    keyPoints: [
...
class Participant:
    ...
    def commit(self, tx_id):
      ...
      },
      {
        ...
        content: 'If coordinator crashes after some participants vote YES but before sending decision, participants are stuck. Can\'t commit (might need abort), can\'t abort (others might commit).',
        points: [
    ...
    diagram: {
      ...
      nodes: [
        { id: 'coord', type: 'default', position: { x: 300, y: 50 }, data: { label: 'Coordinator' }, style: { background: '#dc2626', color: 'white', padding: 15, border: '2px solid #ef4444' } },
        { id: 'p1', type: 'default', position: { x: 100, y: 180 }, data: { label: 'Participant 1' }, style: { background: '#3b82f6', color: 'white', padding: 15 } },
        { id: 'p2', type: 'default', position: { x: 300, y: 180 }, data: { label: 'Participant 2' }, style: { background: '#3b82f6', color: 'white', padding: 15 } },
        { id: 'p3', type: 'default', position: { x: 500, y: 180 }, data: { label: 'Participant 3' }, style: { background: '#3b82f6', color: 'white', padding: 15 } },
      ],
      edges: [
        { id: 'e1', source: 'coord', target: 'p1', label: 'PREPARE', animated: true },
        { id: 'e2', source: 'coord', target: 'p2', label: 'PREPARE', animated: true },
        { id: 'e3', source: 'coord', target: 'p3', label: 'PREPARE', animated: true },
        { id: 'e4', source: 'p1', target: 'coord', label: 'YES', animated: false, style: { stroke: '#10b981' } },
        { id: 'e5', source: 'p2', target: 'coord', label: 'YES', animated: false, style: { stroke: '#10b981' } },
        { id: 'e6', source: 'p3', target: 'coord', label: 'YES', animated: false, style: { stroke: '#10b981' } },
      ]
    }
  },
  {
    ...
    part: 'II',
    title: 'Consensus',
    summary: 'Paxos (classic), Raft (understandable), and Multi-Paxos for multiple rounds. Fundamental problem in distributed systems.',
    detailedContent: 'Consensus allows distributed nodes to agree on a value despite failures. Harder than it looks—took decades to get right.',
    keyPoints: [
    ...
    diagram: {
      ...
      nodes: [
        { id: 'leader', type: 'default', position: { x: 300, y: 50 }, data: { label: '👑 Leader\n(Term 3)' }, style: { background: '#dc2626', color: 'white', padding: 20, border: '3px solid #ef4444' } },
      ...
      edges: [
        { id: 'e1', source: 'leader', target: 'f1', label: 'AppendEntries', animated: true },
        { id: 'e2', source: 'leader', target: 'f2', label: 'AppendEntries', animated: true },
        { id: 'e3', source: 'leader', target: 'f3', label: 'AppendEntries', animated: true },
      ]
    }
  }
];

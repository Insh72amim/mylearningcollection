...
const ComplexAnalysisBook = ({ onBack }) => {
  ...
  const chapters = complexAnalysisChapters;
  ...
  return (
    <div className="w-full max-w-4xl lg:max-w-5xl mx-auto px-4 sm:px-6 lg:px-0 text-gray-300 space-y-8 pb-16 sm:pb-20">
      {/* Header - Indigo Theme for Mathematics */}
      <div className="border-b border-gray-700 pb-8">
        ...
        <div className="flex flex-col gap-4 sm:gap-6 sm:flex-row sm:items-start">
          <div className="w-full sm:w-24 h-24 sm:h-32 bg-gradient-to-br from-indigo-700 to-blue-900 rounded-lg shadow-xl flex items-center justify-center shrink-0 border border-indigo-600">
            <Sigma size={40} className="text-indigo-100" />
          </div>
          <div className="text-center sm:text-left">
            ...
            <p className="text-lg sm:text-xl text-gray-400 mb-4">
              Murray R. Spiegel • Schaum's Outline
            </p>
            <div className="flex flex-wrap justify-center sm:justify-start gap-3">
              <span className="px-3 py-1 rounded-full bg-indigo-900/50 text-indigo-300 text-sm border border-indigo-700">
                Calculus
              </span>
              <span className="px-3 py-1 rounded-full bg-blue-900/50 text-blue-300 text-sm border border-blue-700">
                Analysis
              </span>
              <span className="px-3 py-1 rounded-full bg-violet-900/50 text-violet-300 text-sm border border-violet-700">
                Problems
              </span>
            </div>
          </div>
          ...
          <div
            ...
            <button
              ...
              <div className="flex items-start sm:items-center gap-3 sm:gap-4 flex-1">
                <div className="hidden sm:flex items-center justify-center w-10 h-10 rounded-lg bg-indigo-900/50 text-indigo-400 font-bold text-sm border border-indigo-800 shrink-0">
                  {chapter.id.split("-")[1]}
                </div>
            ...
            {expandedChapters[chapter.id] && (
              <div className="px-4 sm:px-6 pb-6 pt-2 border-t border-gray-700 bg-gray-800/50">
                        ...
                        {section.definitions && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            {section.definitions.map((def, dIdx) => (
                              <div
                                key={dIdx}
                                className="bg-gray-800/80 p-3 rounded border-l-2 border-indigo-500">
                                <div
                                  className="text-indigo-300 font-medium mb-1"
                                  dangerouslySetInnerHTML={{
                ...
                {/* Deep Dive */}
                {chapter.deepDive && (
                  <div className="mt-8 bg-gradient-to-br from-indigo-900/20 to-blue-900/20 border border-indigo-900/50 rounded-xl p-6">

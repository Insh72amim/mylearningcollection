...
import { getGoSectionData } from "../../data/goData";

const GoDocs = ({ onBack, section = "syntax" }) => {
  const [copiedCode, setCopiedCode] = useState("");
  const [selectedTopic, setSelectedTopic] = useState(null);

  const sectionData = getGoSectionData(section);
  ...
  const renderContent = (content) => {
    // Split content by code blocks (cpp, java, python, go)
    const codeBlockRegex = /```(cpp|java|python|go)\n([\s\S]*?)```/g;
    const parts = [];
    let lastIndex = 0;
    let match;
            ...
            .map((line, lineIndex) => {
              ...
              if (line.startsWith("### ")) {
                ...
                const processedLine = line
                  ...
                  .replace(
                    /`([^`]+)`/g,
                    '<code class="bg-gray-700 text-cyan-400 px-2 py-1 rounded text-sm">$1</code>'
                  );
        ...
        <motion.div
          ...
          <div className="flex items-center mb-8">
            <button
              ...
              className="flex items-center text-cyan-400 hover:text-cyan-300 mb-4 transition-colors">
      ...
      <motion.div
        ...
        <div className="text-center mb-12">
          <motion.div
            ...
            className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sectionData.topics.map((topic, index) => (
            <motion.div
              ...
              className="group bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-cyan-500 transition-all duration-300 cursor-pointer hover:shadow-xl hover:shadow-cyan-500/20"
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <ChevronRight className="w-6 h-6 text-gray-500 group-hover:text-cyan-400 transition-colors flex-shrink-0 ml-4" />
              </div>

...
import { getJavaScriptSectionData } from "../../data/javascriptData";

const JavaScriptDocs = ({ onBack, section = "syntax" }) => {
  const [copiedCode, setCopiedCode] = useState("");
  const [selectedTopic, setSelectedTopic] = useState(null);

  const sectionData = getJavaScriptSectionData(section);
  ...
  const renderContent = (content) => {
    const codeBlockRegex = /```(cpp|java|python|go|javascript)\n([\s\S]*?)```/g;
    const parts = [];
    let lastIndex = 0;
    let match;
        ...
        if (part.type === "code") {
          ...
          return part.content
            ...
            .map((line, lineIndex) => {
              ...
              if (line.startsWith("### ")) {
                return (
                  <h3 key={key} className="text-2xl font-bold text-white mt-8 mb-4">
                    {line.replace("### ", "")}
                  </h3>
                );
              } else if (line.startsWith("## ")) {
                return (
                  <h2 key={key} className="text-3xl font-bold text-white mt-10 mb-6">
                    {line.replace("## ", "")}
                  </h2>
                );
              } else if (line.startsWith("# ")) {
                return (
                  <h1 key={key} className="text-4xl font-bold text-white mt-12 mb-8">
                    {line.replace("# ", "")}
                  </h1>
                );
              } else if (line.startsWith("- ")) {
                ...
                const processedLine = line
                  ...
                  .replace(
                    /`([^`]+)`/g,
                    '<code class="bg-gray-700 text-yellow-400 px-2 py-1 rounded text-sm">$1</code>'
                  );
      ...
      <motion.div
        ...
        <div className="text-center mb-12">
          <motion.div
            ...
            className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-yellow-500 to-orange-600 rounded-2xl mb-6">
            <Code className="w-10 h-10 text-white" />
          </motion.div>

          <h1 className="text-5xl font-bold bg-gradient-to-r from-yellow-400 via-orange-400 to-yellow-400 bg-clip-text text-transparent mb-4">
            {sectionData.title}
          </h1>

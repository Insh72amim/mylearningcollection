...
import { graphProblems, graphResources } from '../../data/graphData';
...
const GraphDocs = () => {
  ...
  const allCategories = ['All', ...new Set(graphProblems.map(p => p.category))];

  // Filter problems
  const filteredProblems = graphProblems.filter(problem => {
    ...
    <div className="h-full overflow-y-auto bg-gray-900">
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="border-b border-gray-700 pb-6">
          <h1 className="text-4xl font-bold text-white mb-3">Graphs</h1>
          <p className="text-lg text-gray-400">
            Master graph algorithms: traversal, shortest paths, MST, connectivity, and flow networks.
          </p>
        </div>

        {/* Resources Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {graphResources.map((resource, idx) => (
            <a
              key={idx}
              href={resource.url}
              target="_blank"
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
            ...
            Algorithms & Implementations ({filteredProblems.length})
          </h2>

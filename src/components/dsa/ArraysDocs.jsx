...
import { arraysProblems, arraysResources } from '../../data/arraysData';
...
const ArraysDocs = () => {
  ...
  const allCategories = ['All', ...new Set(arraysProblems.map(p => p.category))];

  const filteredProblems = arraysProblems.filter(problem => {
    ...
    <div className="h-full overflow-y-auto bg-gray-900">
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="border-b border-gray-700 pb-6">
          <h1 className="text-4xl font-bold text-white mb-3">Arrays & Bit Manipulation</h1>
          <p className="text-lg text-gray-400">
            Master array algorithms, bit manipulation tricks, sorting techniques, and classic interview problems.
          </p>
        </div>

        {/* Resources Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {arraysResources.map((resource, idx) => (
            <a
              key={idx}
              href={resource.url}
              target="_blank"

...
import { treeProblems, treeResources } from '../../data/treeData';
...
const TreeDocs = () => {
  ...
  const allCategories = ['All', ...new Set(treeProblems.map(p => p.category))];

  // Filter problems
  const filteredProblems = treeProblems.filter(problem => {
    ...
    <div className="h-full overflow-y-auto bg-gray-900">
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="border-b border-gray-700 pb-6">
          <h1 className="text-4xl font-bold text-white mb-3">Trees & Data Structures</h1>
          <p className="text-lg text-gray-400">
            Master hierarchical data structures: Linked Lists, Tries, Binary Trees, BSTs, and advanced tree structures.
          </p>
        </div>

        {/* Resources Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {treeResources.map((resource, idx) => (
            <a
              key={idx}
              href={resource.url}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-green-500 transition-colors group"
            >
              <div className="flex items-start justify-between mb-2">
                <span className="text-xs font-medium px-2 py-1 rounded bg-green-900/30 text-green-400 border border-green-800/50">
                  {resource.type}
                </span>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-800/50 p-4 rounded-xl border border-gray-700">
          ...
          <input
            ...
            className="w-full md:w-64 bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-green-500"
          />
        </div>
            ...
            >
              ...
              {expandedProblem === problem.id && (
                <div className="px-6 pb-6 pt-2 border-t border-gray-700 bg-gray-900/30">
                  {/* Thought Process */}
                  {problem.thought && (
                    <div className="mb-6 bg-green-900/10 border border-green-800/30 p-4 rounded-lg">
